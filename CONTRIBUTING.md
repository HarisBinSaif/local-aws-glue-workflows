...
v0.4 covers: Terraform input, jobs only, three trigger types (ON_DEMAND, CONDITIONAL, SCHEDULED), two executors (mock + glue-docker), and Terraform parameter declarations honored (`default_run_properties`, `default_arguments`, `actions[].arguments`). Out-of-scope: crawlers, EVENT triggers, job bookmarks, Glue Catalog API mocks. These are deferred to later plans — please open an issue before starting.
